...
type ProductModel = {
  ...
  repoSnapshot?: { headRevision?: string | null; dirty?: boolean; changedFiles?: string[]; capturedAt?: string | null };
...
type ProductMapProjection = {
  ...
  hierarchy: { levels: number; roots: string[]; groups: ProductMapGroup[] };
  visualLayout?: Record<string, { x?: number; y?: number }>;
  layoutAuthority?: string;
  repoSnapshot?: ProductModel["repoSnapshot"];
};

type ProductProposal = { id: string; status: "pending" | "confirmed" | "rejected" | "superseded"; projectId: string; baseRepoSnapshotId: string; proposedByAgentId: string; groups: Array<{ proposalKey: string; name: string; description?: string; memberFeatureKeys?: string[] }>; features: Array<{ proposalKey: string; name: string; description?: string; evidenceRefs: string[]; groupKey?: string | null; rationale?: string }>; version: number; provenance?: { certainty?: string; evidenceRefs?: string[] } };
type ProductResponse = { model: ProductModel; status: "READY" | "PARTIAL" | "FAILED"; understandingState?: UnderstandingState; recommendedNextAction?: string | null; freshness: "CURRENT" | "STALE"; featureCount?: number; proposals?: ProductProposal[] };
...
type Workspace = {
  ...
  entities: {
    ...
    agents: Record<string, { id: string; displayName?: string; status?: string; capabilities?: string[] }>;
    ...
    changes: Record<string, { id: string; title: string; status?: string; featureId?: string | null }>;
...
export function App() {
  ...
  const loadProductModel = useCallback(async (scan = false) => {
    ...
    try {
      ...
      setProduct(result);
      setStatus(result.freshness === "STALE" ? "项目发生变化，需要重新确认" : result.status === "PARTIAL" ? "项目已部分理解" : "项目地图已就绪");
      setError(null);
      return result;
    } catch (cause: any) {
  ...
  useEffect(() => {
    Promise.all([refreshWorkspace(true), loadProductModel()]).catch(() => undefined);
    return () => {
  ...
  const groups = projection?.hierarchy.groups || [];
  ...
  const updateProductModel = (next: ProductModel) => {
    const stale = Boolean(next.repoSnapshot?.dirty) || Object.values(next.features || {}).some((feature) => feature.actionability === "GROUNDED");
    const understandingState = next.understandingState || (Object.keys(next.features || {}).length ? "READY" : "FAILED");
    setProduct({ model: next, status: understandingState === "READY" ? "READY" : understandingState === "FAILED" || understandingState === "UNSUPPORTED" ? "FAILED" : "PARTIAL", understandingState, recommendedNextAction: next.recommendedNextAction, freshness: stale ? "STALE" : "CURRENT", proposals: product?.proposals || [] });
  };
  const correctFeature = async (patch: Record<string, unknown>, successMessage: string) => {
    if (!selectedFeature) return;
    try {
      const result = await request<{ model: ProductModel; feature: Feature }>("/api/session/product-feature/correct", { method: "POST", body: JSON.stringify({ feature_id: selectedFeature.id, expected_version: selectedFeature.version, patch }) });
  ...
  const statusTitle = understandingState === "NEEDS_INTERPRETATION" ? "需要 AI 解释" : understandingState === "UNSUPPORTED" ? "暂不支持" : understandingState === "PARTIAL" ? "部分理解" : understandingState === "FAILED" ? "理解失败" : "项目地图";
  const freshnessTitle = product?.freshness === "STALE" ? "需要重新确认" : "与当前代码同步";
  ...
  return (
    <main className="product-shell">
      ...
      {view === "product" && (product?.proposals || []).filter((proposal) => proposal.status === "pending").map((proposal) => <div className="proposal-card global-proposal" key={proposal.id}><div><strong>AI 根据项目证据整理出 {proposal.features.length} 个产品功能</strong><span>这是 Product Inference 建议，不是代码事实；确认后才会进入产品地图。</span></div><div className="proposal-actions"><button className="primary-button" onClick={() => void applyProposal(proposal, "confirm")}>确认这组结构</button><button className="ghost-button" onClick={() => setStatus("建议可在这里逐项查看；当前保留为待确认")}>逐项查看</button><button className="text-button danger-text" onClick={() => void applyProposal(proposal, "reject")}>拒绝</button></div></div>)}

      <aside className="product-sidebar">
        <div className="sidebar-section"><div className="sidebar-label">项目</div>
          <button className={`sidebar-item ${view === "product" ? "active" : ""}`} onClick={() => setView("product")}><span className="nav-icon">⌂</span><span><strong>Product Map</strong><small>产品能力地图</small></span></button>
          <button className={`sidebar-item ${view === "board" ? "active" : ""}`} onClick={() => setView("board")}><span className="nav-icon">▦</span><span><strong>Board View</strong><small>0.2 语义白板</small></span></button>
          <button className={`sidebar-item ${view === "changes" ? "active" : ""}`} onClick={() => setView("changes")}><span className="nav-icon">↗</span><span><strong>Changes</strong><small>变更入口</small></span></button>
        </div>
