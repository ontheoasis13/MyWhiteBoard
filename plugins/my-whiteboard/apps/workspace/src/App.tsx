...
type ProductResponse = { model: ProductModel; status: "READY" | "PARTIAL" | "FAILED"; freshness: "CURRENT" | "STALE"; featureCount?: number };
...
export function App() {
  ...
  const [selectedKind, setSelectedKind] = useState<"feature" | "group" | null>(null);
  ...
  const updateProductModel = (next: ProductModel) => {
    ...
    setProduct({ model: next, status: Object.keys(next.features || {}).length ? "READY" : "FAILED", freshness: stale ? "STALE" : "CURRENT" });
  };
  ...
  const statusTitle = product?.status === "PARTIAL" ? "部分理解" : product?.status === "FAILED" ? "理解失败" : "项目地图";
