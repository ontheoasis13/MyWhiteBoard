...
type ProductResponse = { model: ProductModel; status: "READY" | "PARTIAL" | "FAILED"; understandingState?: UnderstandingState; recommendedNextAction?: string | null; freshness: "CURRENT" | "STALE"; productStructureStatus?: "PROPOSED" | "AWAITING_HUMAN_CONFIRMATION" | "CONFIRMED" | "REJECTED" | "STALE"; featureCount?: number; proposals?: ProductProposal[] };
...
type Workspace = {
  ...
  entities: {
    ...
    agents: Record<string, { id: string; displayName?: string; status?: string; capabilities?: string[]; lastSeenAt?: string | null; registeredAt?: string | null; lastActivityAt?: string | null; heartbeatAt?: string | null }>;
...
function agentPresence(agent: { status?: string; lastSeenAt?: string | null; lastActivityAt?: string | null; heartbeatAt?: string | null }) {
  const seen = agent.lastActivityAt || agent.lastSeenAt;
  const recent = seen ? Date.now() - new Date(seen).getTime() < 5 * 60 * 1000 : false;
  if (agent.heartbeatAt && Date.now() - new Date(agent.heartbeatAt).getTime() < 90 * 1000) return "实时同步中";
  if (recent && agent.status !== "offline") return "已注册 · 最近活动：刚刚 · 通过 MCP";
  if (seen) return "已注册 · 最近未活动";
  return "已注册";
}
...
export function App() {
  ...
  const loadProductModel = useCallback(async (scan = false) => {
    ...
    try {
      ...
      setProduct(result);
      setStatus(result.freshness === "STALE" ? "代码发生变化，需要重新理解实现依据" : result.productStructureStatus === "AWAITING_HUMAN_CONFIRMATION" ? "产品结构等待你确认" : result.status === "PARTIAL" ? "项目已部分理解" : "项目地图已就绪");
      setError(null);
      return result;
    } catch (cause: any) {
